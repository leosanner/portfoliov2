import { motion } from "framer-motion";
import { Mail, ArrowUpRight } from "lucide-react";
import type { SVGProps } from "react";
import { Footer } from "./Footer";

function LinkedinIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  );
}

function GithubIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  );
}

const contactLinks = [
  {
    icon: Mail,
    label: "Email",
    value: "leosanner12@gmail.com",
    href: "mailto:leosanner12@gmail.com",
    color: "var(--color-primary)",
  },
  {
    icon: LinkedinIcon,
    label: "LinkedIn",
    value: "/in/leonardo-sanner",
    href: "https://www.linkedin.com/in/leonardo-sanner-86b27a305/",
    color: "var(--color-tertiary)",
  },
  {
    icon: GithubIcon,
    label: "GitHub",
    value: "leosanner",
    href: "https://github.com/leosanner",
    color: "var(--color-on-surface)",
  },
];

const linkVariants = {
  hidden: { opacity: 0, y: 30 },
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: {
      delay: 0.3 + i * 0.12,
      duration: 0.5,
      ease: "easeOut" as const,
    },
  }),
};

export function Contact() {
  return (
    <section
      id="contact"
      className="snap-section relative items-center justify-center py-24 md:py-20"
    >
      {/* Radial glow behind heading */}
      <div
        className="pointer-events-none absolute top-1/3 left-1/2 h-100 w-150 -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-full opacity-[0.04]"
        style={{
          background:
            "radial-gradient(circle, var(--color-primary), transparent 60%)",
        }}
      />

      <div className="relative z-10 mx-auto max-w-3xl px-6 text-center">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-100px" }}
          transition={{ duration: 0.6 }}
        >
          <span className="font-label text-xs font-medium uppercase tracking-[0.2em] text-primary">
            Contato
          </span>

          <h2 className="font-headline mt-4 text-3xl font-extrabold leading-[1.1] tracking-tight text-on-surface sm:text-4xl lg:text-6xl">
            Vamos construir algo <span className="text-primary">juntos</span>?
          </h2>

          <p className="mx-auto mt-5 max-w-md text-base leading-relaxed text-on-surface-variant font-body">
            Aberto a oportunidades, colaborações e conversas sobre tecnologia.
          </p>
        </motion.div>

        {/* Contact links as large interactive rows */}
        <motion.div
          className="mt-8 flex flex-col gap-3 sm:mt-14"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-80px" }}
        >
          {contactLinks.map((link, i) => {
            const Icon = link.icon;
            return (
              <motion.a
                key={link.label}
                href={link.href}
                target={link.href.startsWith("mailto") ? undefined : "_blank"}
                rel="noopener noreferrer"
                custom={i}
                variants={linkVariants}
                className="group flex items-center gap-4 rounded-xl border border-outline-variant/30 bg-surface-container-low/50 px-6 py-4 transition-all duration-300 hover:border-primary/30 hover:bg-surface-container"
              >
                <div
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg transition-colors duration-300"
                  style={{
                    backgroundColor: `color-mix(in srgb, ${link.color} 12%, transparent)`,
                  }}
                >
                  <Icon
                    className="h-5 w-5 transition-colors duration-300"
                    style={{ color: link.color }}
                  />
                </div>

                <div className="flex flex-1 flex-col items-start">
                  <span className="font-label text-[11px] uppercase tracking-[0.2em] text-outline">
                    {link.label}
                  </span>
                  <span className="text-sm font-medium text-on-surface">
                    {link.value}
                  </span>
                </div>

                <ArrowUpRight className="h-4 w-4 text-outline transition-all duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-primary" />
              </motion.a>
            );
          })}
        </motion.div>
      </div>

      <Footer />
    </section>
  );
}
